...
describe('LuxonDateAdapter', () => {
  ...
  beforeEach(inject(
    ...
    (dateAdapter: LuxonDateAdapter, p: Platform) => {
      adapter = dateAdapter;
      platform = p;
  ...
  it("should get today's date", () => {
    expect(adapter.sameDate(adapter.today(), DateTime.local())).toBe(
      true,
  ...
  it('should parse Date', () => {
    ...
    expect(adapter.parse(date, 'LL/dd/yyyy')!.toISO()).toEqual(
  ...
  it('should parse invalid value as invalid', () => {
    ...
    expect(adapter.isValid(d as DateTime)).toBe(
      false,
  ...
  it('should format date according to given format', () => {
    expect(adapter.format(DateTime.local(2017, JAN, 2), 'LL/dd/yyyy')).toEqual(
      '01/02/2017'
    );
  });
  ...
  it('should throw when attempting to format invalid date', () => {
    expect(() =>
      adapter.format(DateTime.fromMillis(NaN), 'LL/dd/yyyy')
    ).toThrowError(/LuxonDateAdapter: Cannot format invalid date\./);
  });

  it('should add years', () => {
    expect(
      adapter.addCalendarYears(DateTime.local(2017, JAN, 1), 1).toISO()
  ...
  it('should respect leap years when adding years', () => {
    expect(
      adapter.addCalendarYears(DateTime.local(2016, FEB, 29), 1).toISO()
  ...
  it('should add months', () => {
    expect(
      adapter.addCalendarMonths(DateTime.local(2017, JAN, 1), 1).toISO()
  ...
  it('should respect month length differences when adding months', () => {
    expect(
      adapter.addCalendarMonths(DateTime.local(2017, JAN, 31), 1).toISO()
    ).toEqual(DateTime.local(2017, FEB, 28).toISO());
    expect(
      adapter.addCalendarMonths(DateTime.local(2017, MAR, 31), -1).toISO()
    ).toEqual(DateTime.local(2017, FEB, 28).toISO());
  });

  it('should add days', () => {
    expect(
      adapter.addCalendarDays(DateTime.local(2017, JAN, 1), 1).toISO()
    ).toEqual(DateTime.local(2017, JAN, 2).toISO());
    expect(
      adapter.addCalendarDays(DateTime.local(2017, JAN, 1), -1).toISO()
